...
Примеры запуска:
    python3 generate_big_pg_data.py \
      --dsn "host=127.0.0.1 port=5432 dbname=default_db user=gen_user password=Pass" \
...
def parse_args():
    ...
    p.add_argument("--dsn", required=True, help="Строка подключения libpq, напр.: host=127.0.0.1 port=5432 dbname=default_db user=gen_user password=Pass")
